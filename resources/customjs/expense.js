$(document).ready(function(){
    ...
    /******************************************************Click Event Block*********************************************** */
    /** this will open add expense modal */
    ...
    /**This this will open Edit modal for expense details */
    $('#table_id tbody').on( 'click', '.btn.btn-secondary.action', function () {
      ...
      income_id = data["id"];
       ...
       $('#transaction_date').val(data["expense_date"]);
       $('.btnAdd').hide();
    ...
    /** This will delete expense from expense details */
    $('#table_id tbody').on( 'click', '.btn.btn-danger.action', function () {
      ...
      if (a !="" && a === "YES") {
        let userId = validate($('#e').html());
        formData = { "userId" : userId,  "e"  : "de", "expense_id" : data["id"], }  
        delete_expense(formData) 
        return
      }else{
        alert(" not deleted ")
        return
    ...
    function show_dataTable(){
      return table = $('#table_id').DataTable({
        ajax : "post/expense_post.php?userId="+userId, 
      //main data
        columns: [
          { "data": null },
          { "data": "expense_type" },
          ...
          { "data": "expense_date" },
          {
            "render": function(data, type, full, meta) {
              return  (full["expense_description"].substring(0, 30) ) 
            }
          },
          { "data":  null }
         ] , 
        ...
        success: function(data)
            {
              ...
              switch(data1){
                 ...
                 case "exist": 
                  swal({
                    title: "Info",
                    text: "Sorry! This expense has been added already",
                    icon: "info",
                    });
                    $('.btnAdd').prop("disabled", false)
                 break
    ...
    function delete_expense(formData){
